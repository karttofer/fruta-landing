...
export function paintDocs(el: HTMLElement): Instance {
  return frutaPage(el, {
    ...
    blocks: [
      ...
      { t: 'h', s: '⑮ Audio' },
      { t: 'p', s: 'Web Audio, auto-resumed on the first input (browsers require a gesture). Load and play samples, or synth a quick beep for retro SFX.' },
      { t: 'code', code: "f.beep({ freq: 660, time: 0.08 })\nf.play('jump')\nf.volume(0.6); f.mute()" },
      { t: 'chips', items: ['play', 'beep', 'volume', 'mute'] },
